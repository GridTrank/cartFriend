		...
		methods: {
			...
			imgAdd() {
				...
				uni.chooseImage({
					...
					sourceType: this.sourceType,
					success: (res) => {
						let imagesType=['jpg','png','gif','jpeg']
						let canUpload=res.tempFilePaths.some(item=>{
							return imagesType.some((el)=>{
						...
						this.chooseSuccessMethod(res.tempFilePaths, 0,'photos')
					}
				});
				// #endif
			},
			...
			chooseSuccessMethod(filePaths, type,value) {
